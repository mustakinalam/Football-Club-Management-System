...
<section id = "section1">
        <div class="title" style="color:white";> Shop Staff </div>
			...
			<div style="margin-left:10%;margin-right:10%;margin-top:50px;margin-bottom:50px;text-align:center;background:#b30000;color:white;font-size:large;font-weight: 700;">
			
				<?php
				require_once('DBconnect.php');
					if(isset($_POST['submit-search'])){
						$search = mysqli_real_escape_string($conn,$_POST['search']);
						$sql = "SELECT * FROM shop_staff WHERE staff_id LIKE '%$search%' OR name LIKE '%$search%' OR email LIKE '%$search%' OR salary LIKE '%$search%' OR supervisor LIKE '%$search%' ";
						$result = mysqli_query($conn, $sql); 
						$queryResult = mysqli_num_rows($result);
						echo "---------> There are " .$queryResult." result(s)! <---------<br>"; 
						echo "Staff ID------------Name--------------------Email---------------------Salary(£)---------Supervisor<br>";
						if($queryResult > 0){
							while($row = mysqli_fetch_assoc($result)){
							?>
								<?php echo $row['staff_id'],"------------", $row['name'],"----------", $row['email'],"---------", $row['salary'],"---------------", $row['supervisor'],"<br>" ; ?> 
							<?php
							}
